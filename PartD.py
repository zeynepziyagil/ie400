...
try:
  ...
  #Yi whether product i is taken or not
  Yi=[]
  for k in range(45):
      Yi.append(model.addVar(vtype=GRB.BINARY, name=("Y" + str(k))))
  ...
  Wi=[]
  for i in range(45):
      Wi.append(model.addVar(vtype=GRB.BINARY, name=("W" + str(i))))
  ...
  # disjoint purchase constraint i=22 j=26
  M= 30
  eps = 0.0001
  for i in range(45):
    model.addConstr( Xi[i] >= 0 + eps - M * (1-Yi[i]))
    model.addConstr( Xi[i] <= 0 +  M * Yi[i])
  ...
  # disjoint purchase constraint
  model.addConstr(Yi[22] + Yi[26]<=1)
