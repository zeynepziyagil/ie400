...
try:
  ...
  prd_j_d =26
  #Part e
  Tk = [0,35,45,55,40] #Time spent in markets
  t_d = 300
  ...
  for k in range(5):
    if(k != 0):
      sum_in_markets = sum_in_markets + Vk[k]*Tk[k]
    for l in range(5):
      sum_in_road = sum_in_road + ((dkl[k][l]*Rkl[k][l]) /5)
  model.addConstr(sum_in_road+sum_in_markets <= t_d)
